...
all_camims1 = np.zeros((total_nims, cam1.camdims[1], cam1.camdims[0]), dtype=np.int16)
all_imtimes1 = np.zeros(total_nims)
all_camims2 = np.zeros((total_nims, cam1.camdims[1], cam1.camdims[0]), dtype=np.int16)
...
for k in range(nloops):
    for l in range(n_slmims):
        ...
        all_camims1[count, :, :] = cam1.get_latest_image(return_im=True, waitfornewframe=True)
        all_imtimes1[count] = cam1.loggedims_times_arr[0]
