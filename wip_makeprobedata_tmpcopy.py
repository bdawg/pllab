...
savefilename = 'slmcube_20240605_seeing_0.4-10-scl0.75_rand_10K_01'
...
plotprobeims = None
plotprobeims = [0]
store_complexsine_psfswfs = False #True
...
if enable_seeing:
    ...
    seeing_global_scaling = 0.75 #1

    reset_each_frame = True # No wind, just a new random phase screen each time

    num_timesteps = num_samps
    tvals = np.linspace(0, timespan, num_timesteps)
    dt = np.diff(tvals).mean()
    print('Frame time step = %.3f seconds, windspeed = %.1f m/s' % (dt, speed))
...
for fnum in range(num_files):
    ...
    if enable_seeing:
        ...
        print('Average %.2f ms per screen' % (eltime / num_samps * 1000))
        ...
        all_slmim_params['reset_each_frame'] = reset_each_frame
    else:
        wfcube_seeing = 0
    ...
    wfcube_slm = rad2slm(wfcube, slmrange)
    # if add_127:
    #     wfcube_slm += 127
...
wl=1550
nfrms=100
cube = wfcube_slm / 255. * slmrange
all_rmss = np.zeros(nfrms)
for k in range(nfrms):
    rms = np.std(cube[k,:,:])
    all_rmss[k] = rms
av_rms = np.mean(all_rmss)
av_rms_nm = av_rms / (2*np.pi) * wl
print('Average RMS WF: %.2f rad' % av_rms)
print('Average RMS WF: %.2f nm' % av_rms_nm)
plt.clf()
plt.plot(all_rmss)
plt.ylabel('RMS WF (rad)')
