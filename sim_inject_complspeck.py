...
class multi_complspeckles:
    def __init__ (self, npx_pup, pupildiam_px, padding_px, cropsize=64, randomseed=None, idnum=None):
    ...
    def make_speckles(self, dc_powers=None, cycles_per_pupils=None, ampl1s=None, ampl2s=None, rots=None, phis=None,
        ...
        for k in range(nspeck):
            ...
            self.indiv_puppowers.append(puppower_adj_cur)
    ...
    def measure_fibcoupling(self, orig_field, inp_pix_scale, plot_injfield=False, fignum_inj=3,
        ...
        coupling, mode_coupling, mode_coupling_complex = self.f.calc_injection_multi(mode_field_numbers=self.modes_to_measure,
                                                                verbose=plot_coupling, show_plots=plot_coupling,
                                                                fignum=fignum_coupling, complex=True,
                                                                modes_to_plot=self.modes_to_measure, ylim=None,
                                                                return_abspower=True)
        coeff_vec = np.concatenate((np.real(mode_coupling_complex), np.imag(mode_coupling_complex)))
        # coeff_vec = np.concatenate((np.abs(mode_coupling_complex)**2, np.angle(mode_coupling_complex)))
        return coeff_vec
    ...
    def make_multi_simdata(self, num_data, nspecks, dc_power_range, cycles_per_pupil_range, ampl1_range, ampl2_range,
        ...
        for k in range(num_data):
            if k % 50 == 0:
                if self.idnum is not None:
                    # print('Process %d - Iteration %d' % (self.idnum, k))
                    print('Process %d - Iteration %d - Elapsed time %.1f s' % (self.idnum, k, time.time()-starttime))
                else:
                    print('Iteration %d' % k)
        ...
        if savefile is not None:
            ...
            del all_pupils_out, all_psfs_out, all_slmims_out, all_slm_psfs_out, self.all_psfs, self.all_pupims
            del self.all_slmims, self.all_slm_psfs
            print('Saving done.')
...
if __name__ == '__main__':
    ...
    # import matplotlib.pyplot as plt
    # plt.ion()
    # npf = np.load('/Users/bnorris/DontBackup/PL/iminjout_testsave3.npz')
    # psfs = np.sum(np.abs(npf['all_psfs'])**2, axis=0)
    # plt.clf()
    # # plt.imshow(npf['all_slmims'][0,:,:])
    # plt.imshow(np.sum(np.abs(npf['all_psfs'])**2, axis=0))

    # plt.figure(7)
    # plt.clf()
    # for im in spk.indiv_pupims:
    #     plt.clf()
    #     plt.subplot(121)
    ...
    #     plt.title('Amplitude')
    #     plt.subplot(122)
    #     plt.imshow(np.angle(im), cmap='hsv')
    #     plt.title('Phase')
    #     plt.colorbar()
    #     plt.pause(1)
    # im = spk.all_pupims[-1, :, :]
    ...
    # for l in range(10):
    #     savefile = 'siminjout_4sp_ospl_20230725-01_%.2d.npz' % l
    #     spk.make_multi_simdata(num_data, nspecks, dc_power_range, cycles_per_pupil_range, ampl1_range, ampl2_range,
    #                            rot_range, phi_range, inp_pix_scale, only_one_dc=True, showplots=showplots,
    #                            savefile=savefile, savedir=savedir, make_slmims=make_slmims, singlespeck=singlespeck,
    #                            save_reduced=True)
